...
def cluster(
    ...
    Basic clustering:
    >>> cluster(
        sequence_db="sequences.db",
        cluster_db="results/cluster_db",
        tmp_dir="tmp_cluster",
        s=4.0,
        threads=8
    )
    ...
    >>> cluster(
        sequence_db="large_sequences.db",
        cluster_db="results/gpu_cluster_db",
        tmp_dir="tmp_gpu",
        gpu=True,
        threads=32
    )
    ...
    sequence_db_path = resolve_path(sequence_db, caller_dir)
    cluster_db_path = resolve_path(cluster_db, caller_dir)
    ...
    print(f"MMseqs2 clustering completed. Results saved to: {cluster_db_path}")
